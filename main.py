...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    output = tf.layers.conv2d(vgg_layer7_out, num_classes, 1, strides=(1, 1), kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    ...
    output = tf.add(output, tf.layers.conv2d(vgg_layer4_out, num_classes, 1, strides=(1, 1), kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3)))
    output = tf.layers.conv2d_transpose(output, num_classes, 4, strides=(2, 2), padding='same', kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    tf.Print(output, [tf.shape(output)])

    output = tf.add(output, tf.layers.conv2d(vgg_layer3_out, num_classes, 1, strides=(1, 1), kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3)))
